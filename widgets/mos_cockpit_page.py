...
class MOSCockpitPage(Gtk.ScrolledWindow):
    ...
    def _format(self, data: dict) -> str:
        ...
        lines.append("apex ai lanes:")
        apex = self._read_apex_status()
        if apex:
            overall = apex.get("overall", "UNKNOWN")
            lines.append(f"  overall: {overall}")
            for lane in apex.get("lanes", []):
                status = lane.get("status", "?")
                name = lane.get("name", "?")
                model = lane.get("model", "")
                tps = lane.get("tps")
                gpu = lane.get("gpu", "")
                tps_str = f" tps={tps}" if tps is not None else ""
                gpu_str = f" gpu={gpu}" if gpu else ""
                lines.append(f"  [{status}] {name}: {model}{tps_str}{gpu_str}")
            gw = apex.get("gateway", {})
            lines.append(f"  gateway: {gw.get('status')} port={gw.get('port')} models={gw.get('models')}")
            swarm = apex.get("swarm", {})
            lines.append(f"  swarm: {swarm.get('workersComplete')}/{swarm.get('workersTotal')} complete failures={swarm.get('failures')}")
            if apex.get("next_action"):
                lines.append(f"  next: {apex['next_action']}")
        else:
            lines.append("  ~/.roxy/apex-status.json not found or unreadable")
        lines.append("")
    ...
    @staticmethod
    def _read_apex_status() -> dict | None:
        try:
            path = "/home/mark/.roxy/apex-status.json"
